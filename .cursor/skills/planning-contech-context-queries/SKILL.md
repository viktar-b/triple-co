...
1. Break the task into focused evidence questions.
2. Read `<mount>/profile.md`.
3. Run one focused `smfs grep` before any literal search. Treat budgets
   as hard stops: direct lookup and yes/no trap questions get one
   semantic grep and three shell commands total; cross-chapter synthesis
   ...
   paragraph for the new combined-case concept. When batching, change into
   the mount once.
